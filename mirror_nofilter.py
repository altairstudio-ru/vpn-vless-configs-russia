#!/usr/bin/env python3
# Mirror_all_raw.py — ALL без GEO/тегов, только чистка мусора по протоколу и IP:PORT:SCHEME
...
CLEAN_NOFILTER_DIR = os.path.join(BASE_DIR, "clean_nofilter_all")
NEW_BY_PROTO_DIR = os.path.join(NEW_DIR, "by_protocol_all")
...
def main():
    ...
    urls = load_all_urls()
    print(f"🚀 ALL-RAW старт: источников всего: {len(urls)}")
    ...
    for i, url in enumerate(urls, 1):
        try:
            ...
            for line in lines:
                line = line.strip()
                # единственный фильтр мусора — по протоколу
            ...
            print(f"{i}/{len(urls)}: ✅ {added_local} взято | 🗑️ {trash_local} не протоколы")
    ...
    # сырой ALL без GEO-ограничений
    all_new_all = os.path.join(NEW_DIR, "all_new_all.txt")
    with open(all_new_all, "w", encoding="utf-8") as f:
        f.write("\n".join(all_keys_list))

    # группировка по протоколам (сырой)
    ...
    # общий файл nofilter для ALL
    all_nofilter_all = os.path.join(CLEAN_NOFILTER_DIR, "all_nofilter_all.txt")
    with open(all_nofilter_all, "w", encoding="utf-8") as f:
        f.write("\n".join(clean_keys))

    # по протоколам, чистый ALL
    for p in PROTOCOLS:
    ...
    print("\n✅ ГОТОВО (ALL-RAW, no GEO)")
    print(f"   📥 Всего ключей после протокольного фильтра: {len(all_keys_list)}")
    print(f"   🔗 Уникальных IP:PORT:SCHEME: {len(clean_keys)}")
    print(f"   🗑️ Строк без протокола (мусор): {trash_count}")
